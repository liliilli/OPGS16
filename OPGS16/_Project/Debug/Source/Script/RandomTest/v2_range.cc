...
void Vector2RangeTest::Initiate() {
  ...
  OP16_TIMER_SET(m_timer, 125, true, this, &Vector2RangeTest::Tick);
}

void Vector2RangeTest::Destroy() {
  using opgs16::manager::scene::GetPresentScene;
...
void Vector2RangeTest::Tick() {
  using opgs16::random::ERandomPolicy;
  const auto result = RandomVector2Range(ERandomPolicy::Uniform, 10.f, 15.f);
  m_dist_obj->InsertVector(result);
}

void Vector2RangeTest::Update(float delta_time) {
...
void Vector2RangeTest::Exit() {
  const auto scr = m_object->GetComponent<script::RandomTestManager>();
  if (scr) scr->ReturnFromVector2RangeTest();
