...
///
/// @class CParticleModuleSizeByLife
/// @brief Particle life by size module. (v0.1.0)
///
class CParticleModuleSizeByLife final : public _internal::CInternalParticleModuleBase {
  struct DControlPoint {
    ...
    float GetXTimepoint() const noexcept { return point.x; }
    float GetYScale() const noexcept { return point.y; }
  };
  OP16_SETUP_HASH(CParticleModuleSizeByLife);
public:
  using EParticleProcessType = element::_internal::EParticleProcessType;
  CParticleModuleSizeByLife(CParticleEmitter& emitter);
...
private:
  ...
  float m_start_scale_value = 1.f;
  float m_end_scale_value   = 1.f;
