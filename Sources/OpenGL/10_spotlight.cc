...
int main() {
    ...
    auto shader_object = std::make_unique<helper::Shader>("10_shaders/normal.vert", 
                                                          "10_shaders/spotlight_smooth.frag");
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        for (auto i = 0u; i < 10u; ++i) {
            ...
            shader_object->SetVec3f("Light.direction",  glm::vec3(m_view * glm::vec4(light_front, 0.0)));
            shader_object->SetFloat("Light.inCutoff",   glm::cos(glm::radians(35.f)));
            shader_object->SetFloat("Light.outCutoff",  glm::cos(glm::radians(40.f)));
            shader_object->SetVec3f("Light.ambient",    0.2f * light_color);
            shader_object->SetVec3f("Light.diffuse",    light_color);
            shader_object->SetVec3f("Light.specular",   light_color);
            shader_object->SetFloat("Light.constant",   1.0f);
            shader_object->SetFloat("Light.linear",     0.045f);
            shader_object->SetFloat("Light.quadratic",  0.0075f);
