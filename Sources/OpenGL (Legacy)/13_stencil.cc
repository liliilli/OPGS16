...
int main() {
    ...
    helper::Shader shader_outline{ "11_shaders/normal.vert" , "13_shaders/single_color.frag" };
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        // use outline shader object
        shader_outline.Use();
        shader_outline.SetVecMatirix4f("view", m_view);
        shader_outline.SetVecMatirix4f("projection", camera.GetProjection());

        // set car position and draw
        {
            glm::mat4 m_model;
            m_model = glm::translate(m_model, glm::vec3(0, 0, 1));
            m_model = glm::scale(m_model, glm::vec3(0.041f));
            shader_outline.SetVecMatirix4f("model", m_model);
            car.Draw(shader_outline);
            glBindTexture(GL_TEXTURE_2D, 0);
            glBindVertexArray(0);
        }
        ...
        // set man position and draw
        {
            ...
            m_model = glm::translate(m_model, glm::vec3(0, 0, -2.f));
            m_model = glm::rotate(m_model, glm::radians(-90.f), glm::vec3(0, 1, 0));
            m_model = glm::scale(m_model, glm::vec3(0.20625f));
            shader_outline.SetVecMatirix4f("model", m_model);
            man.Draw(shader_outline);
            glBindTexture(GL_TEXTURE_2D, 0);
            glBindVertexArray(0);
        }
